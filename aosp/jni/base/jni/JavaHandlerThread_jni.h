...
// Step 2: method stubs.
static void InitializeThread(JNIEnv* env, jobject obj,
    jlong nativeJavaHandlerThread,
    jlong nativeEvent) {
  DCHECK(nativeJavaHandlerThread) << "InitializeThread";
  JavaHandlerThread* native =
      reinterpret_cast<JavaHandlerThread*>(nativeJavaHandlerThread);
  return native->InitializeThread(env, obj, nativeEvent);
}

static base::subtle::AtomicWord g_JavaHandlerThread_create = 0;
static ScopedJavaLocalRef<jobject> Java_JavaHandlerThread_create(JNIEnv* env,
    jstring name) {
  /* Must call RegisterNativesImpl()  */
  DCHECK(g_JavaHandlerThread_clazz);
  jmethodID method_id =
